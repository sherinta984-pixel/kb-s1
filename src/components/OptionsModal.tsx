...
export const OptionsModal: React.FC<OptionsModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'themes' | 'teams' | 'text' | 'layout' | 'keys' | 'match' | 'desktop'>('themes');
  ...
  const tabs = [
    ...
    },
    {
      ...
      desc: 'Shortcuts for timer, scoring & controls',
      icon: Keyboard,
    },
    {
      id: 'match' as const,
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
                    <div>
                      ...
                      <p className="text-xs text-slate-400">
                        Click any button and press any keyboard key to instantly rebind.
                      </p>
                    </div>
                    ...
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl bg-white/5 hover:bg-white/10 border border-white/10 text-xs font-semibold text-slate-300 hover:text-white transition-colors cursor-pointer flex items-center gap-2"
                    >
                      <RotateCcw className="w-3.5 h-3.5" />
                  ...
                  <div className="space-y-3 p-4 rounded-2xl bg-blue-950/20 border border-blue-500/20">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-blue-300 flex items-center gap-2">
                        ...
                        {localSettings.team1.name || 'Team 1'} (Left Side) Hotkeys
                      </h4>
                      <span className="text-[10px] font-mono text-blue-400/80">Add • Minus • Reset</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <KeyBindingRow
                        label={`${localSettings.team1.name} Add (+1)`}
                        description="Awards 1 point to left team"
                        currentKey={localSettings.keyBindings.team1Add}
                        onUpdateKey={(key) => handleUpdateKey('team1Add', key)}
                      />
                      <KeyBindingRow
                        label={`${localSettings.team1.name} Minus (-1)`}
                        description="Deducts 1 point from left team"
                        currentKey={localSettings.keyBindings.team1Sub}
                        onUpdateKey={(key) => handleUpdateKey('team1Sub', key)}
                      />
                      <KeyBindingRow
                        label={`${localSettings.team1.name} Reset (0)`}
                        description="Instantly resets left team score to 0"
                        currentKey={localSettings.keyBindings.team1Reset || 'z'}
                        onUpdateKey={(key) => handleUpdateKey('team1Reset', key)}
                      />
                    </div>
                  </div>

                  {/* Section 2: Team 2 Controls */}
                  <div className="space-y-3 p-4 rounded-2xl bg-red-950/20 border border-red-500/20">
                    <div className="flex items-center justify-between">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-red-300 flex items-center gap-2">
                        <span className="w-2.5 h-2.5 rounded-full bg-red-500 shadow-[0_0_8px_#ef4444]" />
                        {localSettings.team2.name || 'Team 2'} (Right Side) Hotkeys
                      </h4>
                      <span className="text-[10px] font-mono text-red-400/80">Add • Minus • Reset</span>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <KeyBindingRow
                        label={`${localSettings.team2.name} Add (+1)`}
                        description="Awards 1 point to right team"
                        currentKey={localSettings.keyBindings.team2Add}
                        onUpdateKey={(key) => handleUpdateKey('team2Add', key)}
                      />
                      <KeyBindingRow
                        label={`${localSettings.team2.name} Minus (-1)`}
                        description="Deducts 1 point from right team"
                        currentKey={localSettings.keyBindings.team2Sub}
                        onUpdateKey={(key) => handleUpdateKey('team2Sub', key)}
                      />
                      <KeyBindingRow
                        label={`${localSettings.team2.name} Reset (0)`}
                        description="Instantly resets right team score to 0"
                        currentKey={localSettings.keyBindings.team2Reset || 'm'}
                        onUpdateKey={(key) => handleUpdateKey('team2Reset', key)}
                      />
                    </div>
                  </div>

                  {/* Section 3: Raid Timer & 3rd Raid (Do-or-Die) */}
                  <div className="space-y-3 p-4 rounded-2xl bg-amber-950/20 border border-amber-500/20">
                    <div className="flex items-center justify-between flex-wrap gap-2">
                      <h4 className="text-xs font-bold uppercase tracking-wider text-amber-300 flex items-center gap-2">
                        ...
                        Raid Timer &amp; 3rd Raid (Do-or-Die) Hotkeys
                      </h4>
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/30">
                        ⚡ Shared Reset Key Active
                      </span>
                    </div>

                    {/* Shared Reset Notice Callout */}
                    <div className="p-3 rounded-xl bg-amber-500/10 border border-amber-500/30 flex items-center gap-2.5 text-xs text-amber-200">
                      ...
                      <span>
                        <b>Shared Reset Architecture:</b> The <b>Raid Reset Key</b> resets both the 30-second countdown clock and deactivates the 3rd Raid (Do-or-Die) state.
                      </span>
                    </div>

                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                      <KeyBindingRow
                        label="Raid Timer Start / Pause"
                        description="Toggles the countdown clock"
                        currentKey={localSettings.keyBindings.raidTimerToggle}
                        onUpdateKey={(key) => handleUpdateKey('raidTimerToggle', key)}
                      />
                      <KeyBindingRow
                        label="3rd Raid (Do-or-Die) Toggle"
                        description="Triggers cinematic Do-or-Die visuals & siren"
                        currentKey={localSettings.keyBindings.thirdRaidToggle || 'd'}
                        onUpdateKey={(key) => handleUpdateKey('thirdRaidToggle', key)}
                      />
                      <KeyBindingRow
                        label="Shared Reset (Clock & 3rd Raid)"
                        description="Resets 30s timer AND clears 3rd raid mode"
                        currentKey={localSettings.keyBindings.raidTimerReset}
                        onUpdateKey={(key) => handleUpdateKey('raidTimerReset', key)}
                      />
                    </div>

                    {/* Fine-tuning Raid Timer Adjustments */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-2 border-t border-white/5">
                      <KeyBindingRow
                        label="Raid Timer (+5s)"
                        description="Adds 5 seconds to current countdown"
                        currentKey={localSettings.keyBindings.raidTimerAdd5}
                        onUpdateKey={(key) => handleUpdateKey('raidTimerAdd5', key)}
                      />
                      <KeyBindingRow
                        label="Raid Timer (-5s)"
                        description="Deducts 5 seconds from current countdown"
                        currentKey={localSettings.keyBindings.raidTimerSub5}
                        onUpdateKey={(key) => handleUpdateKey('raidTimerSub5', key)}
                      />
                    </div>
                  </div>

                  {/* Section 4: Arena Display & Match Controls */}
                  <div className="space-y-3">
                    <h4 className="text-xs font-bold uppercase tracking-wider text-slate-400">
                      Match &amp; Display Hotkeys
                    </h4>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      <KeyBindingRow
                        label="Reset Match Scores (0 - 0)"
                        description="Zeros out scores for second half or new match"
                        currentKey={localSettings.keyBindings.resetScores}
                        onUpdateKey={(key) => handleUpdateKey('resetScores', key)}
                      />
                      <KeyBindingRow
                        label="Toggle Fullscreen"
                        description="Expands to full TV arena without browser borders"
                        currentKey={localSettings.keyBindings.toggleFullscreen}
                        onUpdateKey={(key) => handleUpdateKey('toggleFullscreen', key)}
                      />
                      <KeyBindingRow
                        label="Mute / Unmute Audio"
                        description="Quick toggle for horn buzzer & stadium sound"
                        currentKey={localSettings.keyBindings.toggleSound}
                        onUpdateKey={(key) => handleUpdateKey('toggleSound', key)}
                      />
                      <KeyBindingRow
                        label="Open / Close Settings"
                        description="Opens this configuration console"
                        currentKey={localSettings.keyBindings.openSettings}
                        onUpdateKey={(key) => handleUpdateKey('openSettings', key)}
                      />
                    </div>
                  </div>
                </div>
