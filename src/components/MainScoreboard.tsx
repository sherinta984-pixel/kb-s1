...
export const MainScoreboard: React.FC<MainScoreboardProps> = ({
  ...
  return (
    ...
    >
      ...
      {/* 3rd Raid (Do-or-Die) Warning Message Overlay */}
      <DoOrDieOverlay
        isActive={Boolean(isThirdRaid)}
        showWarning={Boolean(showDoOrDieWarning)}
        onDismissWarning={onDismissDoOrDieWarning || (() => {})}
        onResetBoth={onResetRaidTimerAndThirdRaid}
        resetKeyLabel={settings.keyBindings.raidTimerReset === ' ' ? 'SPACE' : settings.keyBindings.raidTimerReset.toUpperCase()}
        thirdRaidKeyLabel={settings.keyBindings.thirdRaidToggle ? settings.keyBindings.thirdRaidToggle.toUpperCase() : 'D'}
      />
          ...
          {isThirdRaid ? (
            <div className="flex items-center gap-2 text-amber-300 font-bold">
              ...
              <span className="text-slate-200">
                SHARED RESET: PRESS <kbd className="px-2 py-0.5 rounded bg-white/20 text-yellow-300 font-bold">{settings.keyBindings.raidTimerReset.toUpperCase()}</kbd> TO RESET CLOCK &amp; 3RD RAID
              </span>
            </div>
          ) : (
            <span className="text-slate-300 flex items-center gap-2 flex-wrap">
              <span>
                T1: <b className="text-blue-400">{settings.keyBindings.team1Add.toUpperCase()}</b> (+1) / <b className="text-blue-300">{settings.keyBindings.team1Sub.toUpperCase()}</b> (-1) / <b className="text-blue-200">{settings.keyBindings.team1Reset?.toUpperCase() || 'Z'}</b> (RST)
              </span>
              <span className="text-white/20">|</span>
              <span>
                T2: <b className="text-red-400">{settings.keyBindings.team2Add.toUpperCase()}</b> (+1) / <b className="text-red-300">{settings.keyBindings.team2Sub.toUpperCase()}</b> (-1) / <b className="text-red-200">{settings.keyBindings.team2Reset?.toUpperCase() || 'M'}</b> (RST)
              </span>
              <span className="text-white/20">|</span>
              <span>
                RAID: <b className="text-yellow-400">{settings.keyBindings.raidTimerToggle === ' ' ? 'SPACE' : settings.keyBindings.raidTimerToggle.toUpperCase()}</b> (START/PAUSE) / <b className="text-orange-400">{settings.keyBindings.thirdRaidToggle?.toUpperCase() || 'D'}</b> (3RD RAID)
              </span>
              <span className="text-white/20">|</span>
              <span>
                RESET: <b className="text-white bg-white/20 px-1.5 py-0.5 rounded">{settings.keyBindings.raidTimerReset.toUpperCase()}</b> (SHARED)
              </span>
            </span>
          )}
        </div>
